...
describe("generateDirectoryLineData", () => {
  const sampleDirectories: Directory[] = [
    ...
    { dirName: "file1.txt", depth: 2, isLast: true, path: "/folder1/file1.txt" },
    { dirName: "folder2", depth: 1, isLast: true, path: "/folder2" },
    { dirName: "file2.txt", depth: 2, isLast: true, path: "/folder2/file2.txt" },
  ];
  ...
  it("無効なインデックスに対してエラーをスローすること", () => {
    expect(() => generateDirectoryLineData(sampleDirectories, 10 as any)).toThrow();
  });

  it("空のディレクトリ配列に対してエラーをスローすること", () => {
    expect(() => generateDirectoryLineData([], 0 as any)).toThrow();
  });
});